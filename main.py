...
# Dictionnaire pour stocker les zones UI par client
client_ui_areas = {}

def get_client_ui_areas(client: Client):
    if client.id not in client_ui_areas:
        client_ui_areas[client.id] = {
    ...
    return client_ui_areas[client.id]

def set_step(client: Client, s, data=None):
    state = client.user_data.get('state', AppState())
    state.step = s
    if data:
        ...
        if 'colonne_metier' in data:
            ...
            state.code_metier_affiche = mapping.get(col, "CCN 3239")
        if 'art_cible' in data:
            state.art_cible = data['art_cible']
        if 'annexe_id' in data:
            state.annexe_selectionnee = data['annexe_id']
    client.user_data['state'] = state
    build_ui.refresh()
...
def render_result(num_article, txt, state):
    ...
    for art in articles:
        with ui.column().classes('article-card w-full'):
            ...
            res_col = 'texte_simplifie' if state.lang == 'FR' else 'texte_simplifie_en'
            if art[res_col]:
...
@ui.refreshable
def build_ui():
    client = Client.from_client()
    ui_areas = get_client_ui_areas(client)
    header_area = ui_areas['header_area']
    content_area = ui_areas['content_area']
    state = client.user_data.get('state', AppState())
    ...
    UI_TEXT = {
        'FR': {
            'home': '🏠 ACCUEIL',
            'search_label': '🔍 Recherche directe (Ex: 139)',
            'search_btn': 'Aller',
            ...
            'official': '⚖️ Texte officiel'
        },
        'EN': {
            'home': '🏠 HOME',
            'search_label': '🔍 Direct search (Ex: 139)',
            'search_btn': 'Go',
            ...
            'back': '⬅️ BACK',
            'official_pdf': '📄 View Official PDF',
            'official': '⚖️ Official text'
        }
    }
    ...
    with content_area:
        ...
        if state.step not in ['DIRECT', 6, 'LISTE_ANNEXES', 'VOIR_ANNEXE']:
            with ui.expansion(txt['search_label']).classes('w-full border-2 rounded-2xl mb-2 bg-white'):
                with ui.row().classes('w-full items-center p-3'):
                    ...
                    ui.button(txt['search_btn'], on_click=lambda: set_step(client, 'DIRECT', {'art_cible': s_input.value})).props('flat').classes('font-bold')
        ...
        if state.step == 1:
            with ui.column().classes('w-full items-center zoom-page'):
                ...
                with ui.element('div').classes('grid-container w-full'):
                    for m in METIERS_DATA:
                        ...
                        with ui.card().classes('w-full bg-white cursor-pointer p-4 transition-all') \
                            .on('click', lambda m=m, l=label_affiche: set_step(client, 2, {'colonne_metier': m['c'], 'label_metier': l})):
                ...
                ui.button(txt['annexes_btn'], on_click=lambda: set_step(client, 'LISTE_ANNEXES')) \
        ...
        # --- ETAPES SUIVANTES ---
            ...
            with ui.element('div').classes('grid-container w-full'):
                for row in rows:
                    ...
                    with ui.card().classes('w-full h-28 bg-white text-black border-2 border-slate-300 rounded-2xl shadow-md hover:bg-blue-50 cursor-pointer p-0 overflow-hidden') \
                        .on('click', lambda n=n: set_step(client, 'VOIR_ANNEXE', {'annexe_id': n})):
            ...
            with ui.column().classes('w-full'):
                for o in options:
                    ...
                    ui.button(label_bouton, on_click=lambda o=o: set_step(client, 3, {'etape_val': o})).classes(css.BTN_STYLE)
            ui.button(txt['back'], on_click=lambda: set_step(client, 1)).props('flat').classes('w-full mt-4')
            ...
            with ui.column().classes('w-full'):
                for f_v in fams: ui.button(f_v, on_click=lambda f_v=f_v: set_step(client, 4, {'famille_val': f_v})).classes(css.BTN_STYLE)
            ui.button(txt['back'], on_click=lambda: set_step(client, 2)).props('flat').classes('w-full mt-4')
            ...
            with ui.column().classes('w-full'):
                for t in thms: ui.button(t, on_click=lambda t=t: set_step(client, 5, {'theme': t})).classes(css.BTN_STYLE)
            ui.button(txt['back'], on_click=lambda: set_step(client, 3)).props('flat').classes('w-full mt-4')
            ...
            conn.close()
            for q in questions:
                ui.button(q[1], on_click=lambda q_id=q[0]: set_step(client, 6, {'id_question': q_id})).classes('w-full h-auto py-4 bg-white text-black border-2 border-slate-200 rounded-xl px-4 text-left mb-3 font-medium')
            ui.button(txt['back'], on_click=lambda: set_step(client, 4)).props('flat').classes('w-full mt-4')

        elif state.step == 6:
            num_art = db.get_article_from_question(state.choix['id_question'], state.choix['colonne_metier'])
            render_result(num_art, txt, state)
            ui.button(txt['back'], on_click=lambda: set_step(client, 5)).props('flat').classes('w-full mt-4')
...
def main_page(client: Client):
    client.user_data['state'] = AppState()
    build_ui()
