    ...
    with c_zone:
        if state.step == 0:
            with ui.column().classes('w-full items-center justify-start no-wrap h-screen -mt-[95px] p-0 bg-white'):
                # On s'assure que le clic déclenche bien un rafraîchissement global
                def start_app():
                    state.step = 1
                    build_ui.refresh()
        ...
        if state.step == 1:
            # --- BLOC DIALOGUE ---
            with ui.dialog() as direct_dialog, ui.card().classes('items-center w-80 p-8 rounded-2xl').style('min-height: 400px; display: flex; flex-direction: column; justify-content: space-between;'):
            # Titre bien visible
                ui.label(txt['search_label']).classes('font-bold text-center text-slate-800 text-2xl')
            
            # SAISIE DU NUMÉRO : Taille XXL (4rem) et centré
                i_direct = ui.input(placeholder="---") \
                    .classes('w-full mt-4') \
                    .style('font-size: 4rem !important; text-align: center; font-weight: bold; height: auto;')
            
                with ui.column().classes('w-full items-center gap-4 mt-auto'):
                    # Bouton ALLER : Bleu très foncé #1e3a8a
                    ui.button(txt['search_btn'], on_click=lambda: (set_step('DIRECT', {'art_cible': i_direct.value}), direct_dialog.close())) \
                        .classes('w-full py-4 rounded-xl text-white font-bold') \
                        .style('background-color: #1e3a8a !important; font-size: 1.5rem; text-transform: uppercase;')
                    
                    # Bouton Fermer
                    ui.button('Fermer', on_click=direct_dialog.close).props('flat color=grey').classes('text-lg')
            ...
            with ui.element('div').classes('grid-container w-full'):
                for m in METIERS_DATA:
                    ...
                    if is_special:
                        st = 'border: 3px solid #10b981 !important; border-radius: 20px;'
                        on_click_action = lambda: direct_dialog.open()
                    else:
                        st = 'border: 2px solid #e2e8f0 !important; border-radius: 20px;'
                        on_click_action = lambda m_code=m['c'], m_label=label_affiche: set_step(2, {'colonne_metier': m_code, 'label_metier': m_label})

                    card = ui.card().classes('w-full p-4 shadow-sm cursor-pointer active:opacity-50').style(st)
                    card.on('click', on_click_action)
                    with card:
                        with ui.column().classes('items-center justify-center w-full gap-1 pointer-events-none'):
                            icon_color = "#10b981" if is_special else "black"
                            ui.html(f'<i class="fa-solid {m["icon"]}" style="color: {icon_color}; font-size: 1.2rem;"></i>')
                            
                            if not is_special:
                                ui.label(label_affiche).classes('font-bold text-center text-slate-800 uppercase leading-tight').style('font-size: 0.75rem;')
                            else:
                                ui.label("ARTICLE CCN\n3239\nEN 1 CLIC").classes('text-center text-slate-800 font-bold leading-tight').style('white-space: pre-line; font-size: 0.75rem;')
            
            ui.separator().classes('my-4 w-11/12')
            ui.button(txt['annexes_btn'], on_click=lambda: set_step('LISTE_ANNEXES')) \
                .classes('w-full py-4 shadow-lg rounded-2xl') \
                .style('background-color: #1e3a8a !important; color: white !important; font-weight: bold; text-transform: uppercase;')
